...
class headline_news:
    ...
    def __str__(self):
        ...
        result_str.append('이미지 주소:' + self.img_url)
        result_str.append('관련링크:' + self.rel_links)
        return result_str

    def add_rel_links(self, rel_link):
...
def get_common_news(news_obj):
    ...
    ref = ref.replace('ㆍ', '').strip()
    news = main_news(title, url, ref, img_url)
    rel_links = news_obj.select('.relrate_news .item_relate') 
...
def parse_side_news(bsobj):
    ...
    for s_news_tag in s_news_tag_list:
        ...
        ref = s_news_tag.select('.info_mainnews')[0].text.split('ㆍ')[0].strip()
...
def arg_main():
    ...
    for key in categories:
        ...
        headline_news = parse_head_news(parsed)
        main_news_list = parse_main_news(parsed)
        side_nesw_list = parse_side_news(parsed)

        print('카테고리: ' + key)
        print('헤드라인 뉴스: ' + headline_news)
        print('메인 뉴스: ' + str(main_news_list))
        print('사이드 뉴스: ' + str(side_nesw_list))
